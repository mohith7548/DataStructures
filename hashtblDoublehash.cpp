...
int main(){
	cout<<"```````````This program is to Implement Open Adressing Double Hashing in Hash table`````````````"<<endl;
	int choice,data;
	while(1){
...
void insert(int value){
	int hashval = value%size;int r = 11;
	...
	if(hashtable[hashval].isfull == true) {
		...
		if(hashtable[hashval].isfull == false){
			...
			cout<<"Index not free Bad Hash2 Function"<<endl;
			return;
		}
	}
}
